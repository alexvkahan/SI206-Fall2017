...
# Section Day/Time: Thursday 3:00PM
...
class CardTests(unittest.TestCase):
	...
	def test_War(self):
		w=play_war_game()
		self.assertEqual(type(w), tuple)
		self.assertEqual(type(w[0]), str)

	def test_JackofDiamonds(self):
		c=Card(suit=0, rank=11) #Tests to see if card of suit 0 and rank 11 will print as Jack of Diamonds
		self.assertEqual(c.__str__(), 'Jack of Diamonds')

	def test_SortedCards(self): #Testing to see if sorted_cards method of Deck class returns a deck of 52 cards
		d=Deck()
		d.pop_card #Invoke pop_card to remove cards from deck
		d.pop_card
		d.sort_cards
		self.assertEqual(len(d.cards), 52) #Making sure new deck has 52 cards
